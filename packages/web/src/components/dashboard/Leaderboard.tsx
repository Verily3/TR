    ...
    <div className="mt-6 lg:mt-8">
            ...
            {(['week', 'month', 'all'] as const).map((filter) => (
              <button
                ...
                className={`px-2 sm:px-3 py-1.5 rounded text-xs transition-colors capitalize ${
                  timeFilter === filter ? 'bg-card text-sidebar-foreground shadow-sm' : 'text-muted-foreground'
                }`}
              >
                {filter}
              </button>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  ...
                  {currentUser.change !== 0 && (
                    <div
                      className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-xs ${
                        currentUser.change > 0 ? 'bg-green-500/10 text-green-600' : 'bg-red-500/10 text-red-600'
                      }`}
                    >
                      {currentUser.change > 0 ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
            ...
            <div className="grid grid-cols-3 gap-4 sm:gap-8 text-center sm:text-right">
              <div>
                ...
                <div className="text-lg sm:text-2xl text-sidebar-foreground">{currentUser.points.toLocaleString()}</div>
              </div>
              <div>
      ...
      <div className="mb-4 lg:mb-6 grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
        ...
        <div className="p-3 sm:p-4 bg-card border border-border rounded-xl">
          ...
          <div className="text-xs text-muted-foreground mt-1">{topPerformer?.points.toLocaleString()} points</div>
        </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-4 sm:flex-col sm:text-center mt-2 sm:mt-4">
                <div className={`w-12 h-12 sm:w-16 sm:h-16 rounded-full flex items-center justify-center text-base sm:text-lg shrink-0 sm:mx-auto sm:mb-3 bg-gradient-to-br ${style.gradient} text-white`}>
                ...
                <div className="flex-1 sm:flex-none">
                  ...
                  <div className="flex items-center sm:justify-center gap-1 mb-2">
                    <Award className="w-4 h-4 text-accent" />
                    <span className="text-base sm:text-lg text-sidebar-foreground">{participant.points.toLocaleString()}</span>
                  </div>
                  <div className="h-1.5 bg-background/50 rounded-full overflow-hidden">
                    <div className="h-full bg-accent transition-all" style={{ width: `${participant.progress}%` }} />
                  </div>
                  <div className="text-xs text-muted-foreground mt-1">{participant.progress}% complete</div>
                </div>
              </div>
            </div>
          );
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="text-right shrink-0">
                  <div className="flex items-center gap-1 justify-end">
                    ...
                    <span className="text-sm text-sidebar-foreground">{participant.points.toLocaleString()}</span>
                  </div>
                  {participant.change !== 0 && (
                    <div className={`flex items-center gap-1 justify-end text-xs mt-1 ${participant.change > 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {participant.change > 0 ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
            ...
            {rest.map((participant, index) => (
              ...
              >
                ...
                <div className="col-span-2 flex items-center justify-center">
                  {participant.change !== 0 ? (
                    <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs ${participant.change > 0 ? 'bg-green-500/10 text-green-600' : 'bg-red-500/10 text-red-600'}`}>
                      {participant.change > 0 ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
                      {Math.abs(participant.change)}
                    </div>
                  ) : (
