  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (params?.isTemplate !== undefined)
        searchParams.set('isTemplate', params.isTemplate.toString());
...
export function useCreateAgencyProgram() {
  ...
  return useMutation({
    mutationFn: async (input: CreateAgencyProgramInput) => {
      const response = (await api.post<Program>('/api/agencies/me/programs', input)) as unknown as {
        data: Program;
      };
  ...
  return useMutation({
    mutationFn: async (programId: string) => {
      const response = (await api.post<Program>(
        `/api/agencies/me/programs/${programId}/duplicate`
  ...
  return useMutation({
    mutationFn: async ({ moduleId, input }: { moduleId: string; input: UpdateModuleInput }) => {
      const response = await api.put(
        `/api/agencies/me/programs/${programId}/modules/${moduleId}`,
        input
      );
      return (response as unknown as { data: unknown }).data;
    },
...
export function usePublishAgencyProgram(programId: string | undefined) {
  ...
  return useMutation({
    mutationFn: async () => {
      const response = (await api.put<Program>(`/api/agencies/me/programs/${programId}`, {
        status: 'active',
      })) as unknown as { data: Program };
      return response.data;
    },
...
export function useReorderAgencyTasks(programId: string | undefined, lessonId: string | undefined) {
  ...
  return useMutation({
    mutationFn: async (items: { id: string; order: number }[]) => {
      await api.put(`/api/agencies/me/programs/${programId}/lessons/${lessonId}/tasks/reorder`, {
        items,
      });
    },
    onSuccess: () => {
...
export function useReorderAgencyModules(programId: string | undefined) {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async (items: { id: string; order: number }[]) => {
      await api.put(`/api/agencies/me/programs/${programId}/modules/reorder`, { items });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['agencyProgram', programId] });
    },
...
export function useReorderAgencyLessons(
  ...
  return useMutation({
    mutationFn: async (items: { id: string; order: number }[]) => {
      await api.put(`/api/agencies/me/programs/${programId}/modules/${moduleId}/lessons/reorder`, {
        items,
      });
    },
    onSuccess: () => {
...
export function useAssignProgramToClient() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const response = (await api.post<Program>(`/api/agencies/me/programs/${programId}/assign`, {
        tenantId,
        name,
      })) as unknown as { data: Program };
      return response.data;
