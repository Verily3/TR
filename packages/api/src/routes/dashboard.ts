 ...
 */
dashboardRoutes.get('/learner', requireTenantAccess(), async (c) => {
  ...
  const programModules = await db
    ...
    .from(modules)
    .where(and(eq(modules.programId, primaryEnrollment.programId), eq(modules.depth, 0)))
  ...
  const moduleStatsMap = new Map(moduleLessonStats.map((s) => [s.moduleId, s]));
  ...
  const upcomingItems = await db
    ...
    .leftJoin(
      lessonProgress,
      and(eq(lessonProgress.lessonId, lessons.id), eq(lessonProgress.enrollmentId, enrollments.id))
    )
    .where(
      and(
        inArray(modules.programId, programIds),
        inArray(lessons.contentType, ['assignment', 'goal', 'text_form']),
  ...
  const activeGoals = await db
    ...
    .innerJoin(
      enrollments,
      and(eq(goalResponses.enrollmentId, enrollments.id), eq(enrollments.userId, user.id))
    )
  ...
  const pendingApprovals = await db
    ...
    .innerJoin(
      enrollments,
      and(eq(approvalSubmissions.enrollmentId, enrollments.id), eq(enrollments.userId, user.id))
    )
  ...
  // 7. Summary stats
  const totalPoints = userEnrollments.reduce((sum, e) => sum + (e.pointsEarned || 0), 0);
  const avgProgress =
    userEnrollments.length > 0
      ? Math.round(
          userEnrollments.reduce((sum, e) => sum + (e.progress || 0), 0) / userEnrollments.length
        )
      : 0;
...
/**
 * GET /api/tenants/:tenantId/dashboard/leaderboard
 * Rank all active learners in the tenant by total points earned
 */
dashboardRoutes.get('/leaderboard', requireTenantAccess(), async (c) => {
  const currentUser = c.get('user');
  // tenantId is guaranteed by requireTenantAccess() middleware
  ...
  const rows = await db
    .select({
      ...
      totalPoints: sql<number>`coalesce(sum(${enrollments.pointsEarned}), 0)::int`,
      avgProgress: sql<number>`coalesce(round(avg(${enrollments.progress})), 0)::int`,
    })
    .from(enrollments)
    .innerJoin(programs, eq(enrollments.programId, programs.id))
    ...
    .groupBy(users.id, users.firstName, users.lastName, users.title, users.department)
    .orderBy(sql`sum(${enrollments.pointsEarned}) desc nulls last`)
    .limit(20);
  ...
  const participants = rows.map((r) => ({
    ...
    role: r.title ?? r.department ?? 'Learner',
